...
const contactsReducer = (state = initialState, action) => {
    switch(action.type){
        ...
        case SET_IF_ACTIVE_SEARCH:{
            return{ ...state, ifActiveSearch: action.value}
        }
        case CHANGE_NEXT_PAGE:{
            return {...state, nextPage: action.page}
        }
        case CHANGE_PAGINATION_FLAGS:{
            return {...state, hasNextPage: action.hasNextPage, hasPrevPage: action.hasPrevPage, totalPages: action.totalPages, prevPage: action.prevPage, nextPage: action.nextPage, currPage: action.currPage}
        }
      }
    }
...
export const setContacts = (contacts) => {return {type: SET_CONTACTS, contacts}}
export const setIfActiveSearch = (value) => {return {type: SET_IF_ACTIVE_SEARCH, value}}
...
export const deleteContact=(id,page,limit,search)=>(dispatch,getState)=>{
  ...
  API.deleteContact(id).then(responseDelete=>{
    ...
    if(getState().contacts_state.cards.length < 2){//если удаляем последний елемент страницы,то запрос отправляеться на предедушую страницу
